module Rules
  ...
  def save(achievements, result)
    achievements.empty? ? Achievement.create!(user_id: result.user_id, test_id: result.test_id) : achievements.each{ |achievement| achievement.save } 
  end
  ...
  def category_level_achievement(result) 
    ...
    achievements.size == tests.size + 1
  end
