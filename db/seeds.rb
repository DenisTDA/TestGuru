...
param = []
30.times do |index| param << 
  { title: "Theme_#{index}",
    level: rand(5),
    category_id: categories.shuffle.first.id, 
    author_id: users[1..5].shuffle.first.id }
end
...
param = []
50.times do |index| param <<
  { body: "Text_body_question__#{index}",  
    test_id: tests.shuffle.first.id } 
end
...
200.times do |index|
  ...
  qwestion_id: tests.shuffle.first.id,
